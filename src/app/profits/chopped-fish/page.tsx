...
import { Fish, RefreshCw, Info, ArrowRight, ChevronUp, ChevronDown, AlertCircle, CircleDollarSign, Leaf, Loader2, CircleHelp } from 'lucide-react';
...
export default function ChoppedFishPage() {
  ...
  return (
    ...
    >
              ...
              <thead>
                <tr className="bg-muted text-muted-foreground text-xs uppercase tracking-wider border-b border-border">
                  ...
                  <th className="w-[1%] p-4 font-medium text-right cursor-pointer hover:text-foreground whitespace-nowrap" onClick={() => handleSort('profit')}>
                      <div className="flex items-center justify-end gap-1">
                        <Tooltip content={`Estimated total profit for ${choppedFishRequired.toLocaleString()} chopped fish.`}>
                           <span>Total Profit</span>
                           <CircleHelp className="h-3 w-3 text-slate-600" />
                        </Tooltip>
                      ...
                      <React.Fragment key={row.id}>
                          ...
                          >
                            ...
                            <div className="flex items-center gap-3">
                                <div className="h-10 w-15 bg-muted rounded-lg flex items-center justify-center border border-border relative">
                                    <img src={`https://render.albiononline.com/v1/item/${row.id}`} className="h-15 w-10 object-contain" alt={row.name} />
                                    <div className="absolute -top-2 -left-2 bg-background text-[10px] text-muted-foreground border border-border px-1.5 rounded-full">T{row.tier}</div>
                                </div>
                                <div>
                                    <div className="font-medium text-foreground">{row.name}</div>
                                    {row.isRare && <span className="text-[10px] text-warning uppercase tracking-wide font-bold">Rare</span>}
                                </div>
                            </div>
                            </td>
                            {showPrices && (
                                <td className="p-4 text-right">
                                    <div className="font-mono text-foreground">{row.fishPrice.toLocaleString()}</div>
                                </td>
                            )}
                            <td className="p-4 text-right">
                            <div className="font-mono text-muted-foreground">{row.fishVolume.toLocaleString()}</div>
                            </td>
                            <td className="p-4 text-right">
                            <div className="font-mono text-muted-foreground">{row.choppedYield}</div>
                            </td>
                            <td className="p-4 text-right">
                            <div className="font-mono text-success font-bold">{row.fishRequired.toLocaleString()}</div>
                            </td>
                            {showPrices && (
                                <td className="p-4 text-right">
                                    <div className="font-mono text-foreground">{Math.round(row.pricePerChopped).toLocaleString()}</div>
                                </td>
                            )}
                            <td className="p-4 text-right">
                            ...
                            <div className={`text-xs ${row.profit > 0 ? 'text-success/50' : 'text-destructive/50'}`}>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                                        ...
                                        <div className="space-y-3">
                                            <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-wider flex items-center gap-2">
                                                <CircleDollarSign className="h-4 w-4" /> Cost & Input
                                            </h4>
                                            <div className="bg-muted/50 rounded-lg p-4 border border-border space-y-3">
                                                <div className="flex items-start gap-3">
                                                    <img src={`https://render.albiononline.com/v1/item/${row.id}`} className="h-10 w-10 object-contain bg-background rounded-md p-1 mt-5 border border-border" />
                                                    <div className="w-full">
                                                        <NumberInput 
                                                            label="Fish Price"
                                                            value={row.fishPrice}
                                                            onChange={(val) => handlePriceUpdate(row.id, 'fishPrice', val)}
                                                            className="h-9 text-sm bg-background"
                                                            isCustom={row.isCustomFishPrice}
                                                            onReset={() => handleResetPrice(row.id, 'fishPrice')}
                                                        />
                                                        <div className="flex justify-end mt-1">
                                                            <span className="text-xs text-muted-foreground">24h Vol: {row.fishVolume?.toLocaleString()}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div className="text-xs text-muted-foreground flex justify-between pt-2 border-t border-border">
                                                    <span>Base Cost (x{row.fishRequired.toLocaleString()}):</span>
                                                    <span>{Math.round(row.fishPrice * row.fishRequired).toLocaleString()}</span>
                                                </div>
                                                {useBuyOrder && (
                                                    <div className="text-xs text-yellow-500/80 flex justify-between">
                                                        <span>Buy Order Fee (2.5%):</span>
                                                        <span>{Math.round(row.fishPrice * row.fishRequired * 0.025).toLocaleString()}</span>
                                                    </div>
                                                )}
                                                <div className="text-sm font-bold text-foreground flex justify-between pt-2 border-t border-border">
                                                    <span>Total Cost:</span>
                                                    <span>{Math.round(row.fishCost * row.fishRequired).toLocaleString()}</span>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div className="bg-muted/50 rounded-lg p-4 border border-border space-y-3">
                                                <div className="flex items-start gap-3">
                                                    <img src={`https://render.albiononline.com/v1/item/${CHOPPED_FISH_PRODUCT_ID}`} className="h-10 w-10 object-contain bg-background rounded-md p-1 mt-5 border border-border" />
                                                    <div className="w-full">
                                                        <NumberInput 
                                                            label="Chopped Fish Price"
                                                            value={row.choppedFishPrice}
                                                            onChange={(val) => handlePriceUpdate(row.id, 'choppedFishPrice', val)}
                                                            className="h-9 text-sm bg-background"
                                                            isCustom={row.isCustomChoppedPrice}
                                                            onReset={() => handleResetPrice(row.id, 'choppedFishPrice')}
                                                        />
                                                        <div className="flex justify-end mt-1">
                                                            <span className="text-xs text-muted-foreground">24h Vol: {choppedFishStats.volume.toLocaleString()}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div className="text-xs text-muted-foreground flex justify-between pt-2 border-t border-border">
                                                    <span>Yield (x{row.choppedYield}):</span>
                                                    <span>{choppedFishRequired.toLocaleString()} total</span>
                                                </div>
                                                {includeTax && (
                                                    <div className="text-xs text-red-400/80 flex justify-between">
                                                        <span>Tax & Fees ({(usePremium ? 4 : 8) + 2.5}%):</span>
                                                        <span>-{Math.round(row.choppedFishPrice * choppedFishRequired * ((usePremium ? 0.04 : 0.08) + 0.025)).toLocaleString()}</span>
                                                    </div>
                                                )}
                                                <div className="text-sm font-bold text-success flex justify-between pt-2 border-t border-border">
                                                    <span>Net Revenue:</span>
                                                    <span>{Math.round(row.revenueTotal * row.fishRequired).toLocaleString()}</span>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="space-y-4 bg-muted/30 p-4 rounded-lg border border-border/50">
                                            <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2 flex items-center gap-2">
                                                <Info className="h-4 w-4" /> Profit Analysis
                                            </h4>
                                            
                                            <div className="space-y-3 text-sm">
                                                <div className="flex justify-between">
                                                    <span className="text-muted-foreground">Price per Chopped</span>
                                                    <span className="text-foreground">{Math.round(row.pricePerChopped).toLocaleString()}</span>
                                                </div>
                                                <div className="flex justify-between mt-2 pt-2 border-t border-border/50">
                                                    <span className="text-muted-foreground">Total Profit</span>
                                                    <span className={`font-bold ${row.profit > 0 ? 'text-success' : 'text-destructive'}`}>
                                                        {Math.round(row.profit).toLocaleString()}
                                                    </span>
                                                </div>
                                                <div className="flex justify-between">
                                                    <span className="text-muted-foreground">ROI</span>
                                                    <span className={`${row.roi > 0 ? 'text-success' : 'text-destructive'}`}>
                                                        {row.roi.toFixed(1)}%
                                                    </span>
                                                </div>
                                            </div>
      ...
      <InfoStrip currentPage="profits-chopped-fish" />
    </PageShell>
  );
}
