...
export default function NewThreadClient() {
  ...
  const CATEGORIES: { value: ThreadCategory; label: string; icon: any; color: string; adminOnly?: boolean }[] = [
    { value: 'General', label: t('categories.General'), icon: <MessageCircle className="h-4 w-4" />, color: 'text-blue-400' },
    { value: 'Builds', label: t('categories.Builds'), icon: <Shield className="h-4 w-4" />, color: 'text-orange-400' },
    { value: 'Market', label: t('categories.Market'), icon: <Coins className="h-4 w-4" />, color: 'text-yellow-400' },
    { value: 'PvP', label: t('categories.PvP'), icon: <Sword className="h-4 w-4" />, color: 'text-red-400' },
    { value: 'Guilds', label: t('categories.Guilds'), icon: <Users className="h-4 w-4" />, color: 'text-purple-400' },
    { value: 'Recruitment', label: t('categories.Recruitment'), icon: <Target className="h-4 w-4" />, color: 'text-green-400' },
    { value: 'LFG', label: t('categories.LFG'), icon: <Trophy className="h-4 w-4" />, color: 'text-pink-400' },
    { value: 'Trading', label: t('categories.Trading'), icon: <Coins className="h-4 w-4" />, color: 'text-emerald-400' },
    { value: 'Guides', label: t('categories.Guides'), icon: <Target className="h-4 w-4" />, color: 'text-indigo-400' },
    { value: 'Announcements', label: t('categories.Announcements'), icon: <AlertCircle className="h-4 w-4" />, color: 'text-red-500', adminOnly: true },
    { value: 'Official', label: t('categories.Official'), icon: <Shield className="h-4 w-4" />, color: 'text-blue-500', adminOnly: true },
    { value: 'News', label: t('categories.News'), icon: <AlertCircle className="h-4 w-4" />, color: 'text-sky-400', adminOnly: true },
  ];

  const SERVERS: { value: ServerRegion; label: string; icon: any }[] = [
    { value: 'All', label: t('servers.All'), icon: <Globe className="h-4 w-4" /> },
    { value: 'Americas', label: t('servers.Americas'), icon: <Globe className="h-4 w-4 text-blue-400" /> },
    { value: 'Asia', label: t('servers.Asia'), icon: <Globe className="h-4 w-4 text-yellow-400" /> },
    { value: 'Europe', label: t('servers.Europe'), icon: <Globe className="h-4 w-4 text-emerald-400" /> },
  ];
            ...
            <div className="bg-card border border-border rounded-2xl p-6 space-y-6">
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <label className="text-xs uppercase tracking-widest text-muted-foreground">{t('threadTitle')}</label>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <label className="text-xs uppercase tracking-widest text-muted-foreground">{t('content')}</label>
                    <span className="text-[10px] text-muted-foreground font-bold">
                      {t('characters', { count: (content?.length || 0).toLocaleString() })}
                    </span>
                  </div>
                  <button
                    type="button"
                  ...
                  >
                    ...
                    {preview ? t('editMode') : t('livePreview')}
                  </button>
                </div>
                ...
                <p className="text-[10px] text-muted-foreground italic">{t('markdownNote')}</p>
              </div>
                ...
                <div className="flex flex-wrap gap-2 mb-2">
                  ...
                  {tags.length < 5 && (
                    <input
                      ...
                      placeholder={tags.length === 0 ? t('addTags') : t('addMore')}
                      className="bg-transparent border-none outline-none text-xs text-foreground min-w-[120px]"
                    />
                  )}
                </div>
                <p className="text-[10px] text-muted-foreground">{t('tagsNote')}</p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between gap-4 p-4 bg-card border border-border rounded-xl">
              <div className="flex items-center gap-3">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">
                  {title && content ? t('readyToShare') : t('awaitingInput')}
                </span>
              </div>
              <div className="flex items-center gap-3">
                <button
                ...
                <div className="space-y-4">
                  ...
                  {category === 'Recruitment' && (
                    <div className="space-y-1.5 animate-in fade-in slide-in-from-top-1">
                      <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">{t('guildName')}</label>
                      <input
                        required
                        type="text"
                        value={guildName}
                        onChange={(e) => setGuildName(e.target.value)}
                        placeholder={t('guildPlaceholder')}
                  ...
                  {category === 'LFG' && (
                    <div className="space-y-1.5 animate-in fade-in slide-in-from-top-1">
                      <label className="text-[13px] text-muted-foreground">{t('activityType')}</label>
                      <input
                        required
                        type="text"
                        value={activityType}
