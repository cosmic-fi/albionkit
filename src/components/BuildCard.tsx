...
export function BuildCard({ build, compactMode = false }: BuildCardProps) {
  ...
  return (
    <div 
        ...
        className={`bg-card/50 border border-border rounded-xl ${compactMode ? 'p-3' : 'p-4'} hover:border-primary/50 hover:shadow-lg hover:shadow-primary/5 transition-all cursor-pointer group h-full flex flex-col relative`}
    >
        <div className={`flex items-start justify-between ${compactMode ? 'mb-2' : 'mb-4'}`}>
          <div className="flex items-center gap-3">
            <div className="relative">
                ...
                {build.items.OffHand && (
                    <div className={`absolute -bottom-0.5 -right-2 ${compactMode ? 'w-6 h-6' : 'w-8 h-8'} rounded-md bg-card border border-border z-10 overflow-hidden shadow-lg`}>
          ...
          {(build.tags?.length || 0) > 2 && (
            <span className="px-2 py-0.5 bg-muted/50 text-muted-foreground text-[10px] rounded border border-border cursor-help relative group/tooltip">
              ...
              <div className="absolute bottom-full right-0 mb-2 hidden group-hover/tooltip:block z-50 w-max max-w-[200px]">
                <div className="bg-popover border border-border rounded p-2 shadow-xl text-xs text-popover-foreground flex flex-wrap gap-1 justify-end">
